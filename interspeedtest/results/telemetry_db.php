 ...
 */
function insertSpeedtestUser($ip, $ispinfo, $extra, $ua, $lang, $dl, $ul, $ping, $jitter, $log, $userid, $subnet, $apname)
{
    $pdo = getPdo();
    ...
    try {
        ...
        (ip,ispinfo,extra,ua,lang,dl,ul,ping,jitter,log,userid,subnet, apname)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)'
        );
        $stmt->execute([
            $ip, $ispinfo, $extra, $ua, $lang, $dl, $ul, $ping, $jitter, $log, $userid, $subnet, $apname
...
function getSpeedtestUserById($id)
{
    ...
    try {
        $stmt = $pdo->prepare(
            'SELECT
            id, timestamp, ip, ispinfo, ua, lang, dl, ul, ping, jitter, log, extra, userid, subnet, apname
